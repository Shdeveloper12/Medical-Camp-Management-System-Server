  ...
  try {
    ...
    app.put("/camps/:id", verifyJWT, async (req, res) => {
      ...
      try {
        ...
        // Validation
        if (!campName || !campFees || !dateTime || !location || !healthcareProfessional || !targetAudience || !description || !specializedServices) {
          return res.status(400).json({ error: "All required fields must be provided" });
        }

        // Check if camp exists and belongs to organizer
        const existingCamp = await campCollection.findOne({ _id: new ObjectId(campId) });
        ...
        console.log('Checking ownership: existingCamp.organizerEmail =', existingCamp.organizerEmail, 'vs req.decoded.email =', req.decoded.email);
        ...
        const updateData = {
          ...
          targetAudience,
          description,
          specializedServices: Array.isArray(specializedServices) ? specializedServices : specializedServices.split(',').map(s => s.trim()),
          updatedAt: new Date()
        };
